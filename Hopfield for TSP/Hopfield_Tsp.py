...
def Distance(Data):
    ...
    for i in range(N):
        for j in range(N):
            Dis[i, j] = np.sqrt((Data[i, 0]-Data[j, 0]) **
                                2+(Data[i, 1]-Data[j, 1])**2)
    return Dis
        ...
        for i in range(N):
            for y in range(N):
                for j in range(N):
                    ...
                    W[x, i, y, j] = -A*delta(x, y)*(1-delta(i, j))-B*delta(i, j)*(
                        1-delta(x, y))-C-D*Dis[x, y]*(delta(j, i+1)+delta(j, i-1))
    return W
...
Data = np.matrix([[1, 2], [2, 1], [1, 3], [2, 4],
                  [3, 1], [4, 2], [3, 4], [4, 3]])
Dis = Distance(Data)
N = len(Dis)
A = N*N
